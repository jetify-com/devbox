...
func RunExamplesTestscripts(t *testing.T, examplesDir string) {
	err := filepath.WalkDir(examplesDir, func(path string, entry os.DirEntry, err error) error {
		...
		skipList := []string{
			...
			//             ".nix-mix/elixir/1-14/rebar3 bare compile --paths $WORK/_build/dev/lib/*/ebin" command failed.
			//             Errors may have been logged above. You can recompile this dependency with "mix deps.compile ranch",
