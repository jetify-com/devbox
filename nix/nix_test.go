...
func TestParseVersionInfoShort(t *testing.T) {
	cases := []struct {
		...
		// Newer Nix releases drop the patch component.
		{"nix (Nix) 2.33", "nix", "2.33"},
...
func TestVersionInfoAtLeast(t *testing.T) {
	...
	// https://github.com/jetify-com/devbox/issues/2766
	// Newer Nix releases drop the patch component, including in prereleases.
	info.Version = "2.33"
	if !info.AtLeast(Version2_18) {
		t.Errorf("got %s < %s", info.Version, Version2_18)
	}
	...
	if !info.AtLeast(MinVersion) {
		t.Errorf("got %s < %s (MinVersion)", info.Version, MinVersion)
	}
